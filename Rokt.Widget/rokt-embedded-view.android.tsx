...
class RoktEmbeddedView extends Component<RoktEmbeddedViewProps, RoktEmbeddedViewState> {
    ...
    onWidgetHeightChanged(event: React.SyntheticEvent) {
      const nativeEvent = event.nativeEvent as HeightChangedEvent
      this.setState({height: parseInt(nativeEvent.height)});
     }

     onWidgetMarginChanged(event: React.SyntheticEvent) {
      const nativeEvent = event.nativeEvent as MarginChangedEvent
      this.setState({
        marginTop: parseInt(nativeEvent.marginTop),
        marginLeft: parseInt(nativeEvent.marginLeft),
        marginRight: parseInt(nativeEvent.marginRight),
        marginBottom: parseInt(nativeEvent.marginBottom)
      })
  }
